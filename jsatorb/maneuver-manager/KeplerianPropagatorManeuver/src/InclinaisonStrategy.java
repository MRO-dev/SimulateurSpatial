...
// bring any real number into [0, 2π)
        INC_2 = ((INC_2 % TWOPI) + TWOPI) % TWOPI;
...
//        }
    ...
    private void normaliseInputAngles() {
        System.out.println("normaliseInputAngles");
        final double TWOPI = 2.0 * FastMath.PI;

        // 1. wrap i and Ω into [0, 2π)
        INC_2 = ((INC_2 % TWOPI) + TWOPI) % TWOPI;
        RAAN  = ((RAAN  % TWOPI) + TWOPI) % TWOPI;

        // 2. mirror inclinations > π back into [0, π]
        if (INC_2 > FastMath.PI) {
            INC_2 = TWOPI - INC_2;
            RAAN  = RAAN + FastMath.PI;
        }
...
//            }
    ...
    private static Vector3D rotateAroundAxis(Vector3D v, Vector3D axis, double angle) {
        return v.scalarMultiply(FastMath.cos(angle))
                .add(axis.crossProduct(v).scalarMultiply(FastMath.sin(angle)))
                .add(axis.scalarMultiply(
                        Vector3D.dotProduct(axis, v) * (1 - FastMath.cos(angle))));
    }
    ...
    public void computeAndExecute() throws IOException, ParseException {
        ...
        System.out.println("Current inc= " + FastMath.toDegrees(currentInc)
                + "°, ECC= " + ECC);
        System.out.println("INC_2= " + FastMath.toDegrees(INC_2) + "°");
        ...
        if (currentInc <= incEps && ECC >= eccThreshold) {
            ...
            // 1) exact apogee epoch
            AbsoluteDate apoDate = findNextApogee(initialState);

// 2) rotate velocity vector
            SpacecraftState apoState = propagateToDate(initialOrbit, apoDate);
            Vector3D rHat  = apoState.getPVCoordinates().getPosition().normalize();
            Vector3D vApo  = apoState.getPVCoordinates().getVelocity();
            double   dI    = INC_2 - new KeplerianOrbit(apoState.getOrbit()).getI();

            Rotation rot   = new Rotation(rHat, dI, RotationConvention.VECTOR_OPERATOR);
            Vector3D vRot  = rot.applyTo(vApo);                            // full, norm‑preserving rotation
            Vector3D deltaV = vRot.subtract(vApo);

// 3) burn exactly at apogee
            ImpulseManeuver burn = new ImpulseManeuver(new DateDetector(apoDate), deltaV, ISP);
            prop = new KeplerianPropagator(initialOrbit);
            prop.addEventDetector(burn);

// 4) post‑burn state **at the same epoch**
            SpacecraftState stateAfterBurn = prop.propagate(apoDate);            // no 1 ms shift!



            double dvApogee = deltaV.getNorm();
        ...
        // Case B: near eq + low e => immediate single-burn
        else if (FastMath.toDegrees(currentInc) <= 0.1) {
            System.out.println("Case B: near-eq + low e => immediate single-burn.");
            System.out.println("Inc = " + FastMath.toDegrees(currentInc) + "°");
            System.out.println("Current Inc " + currentInc+ "°");
            ...
            double nodeSense = FastMath.signum( velocity.getZ() );
            ...
            Vector3D rHat = position.normalize();
            double deltaI = INC_2 - currentInc;

            Rotation rot = new Rotation(rHat, deltaI, RotationConvention.VECTOR_OPERATOR);
            Vector3D vRot = rot.applyTo(velocity);
            Vector3D deltaVVector = vRot.subtract(velocity);
//            Vector3D deltaVVector = vDesired.subtract(velocity);
            System.out.println(" - timeIntermediateParametersFile => ManeuverType: "
                    + MANEUV_TYPE + ", Target Inclination: "
                    + FastMath.toDegrees(INC_2) + "°  (normalised)");
...
// recombine and compute ΔV
            ...
            if (incError <= 0.5) {
                ...
                SpacecraftState stateInitial = new SpacecraftState(initialOrbit, initialMass);
                logState("stateInitial", stateInitial);
                SpacecraftState eqState = eqProp.propagate(eqCrossingDate);
                logState("eqState", eqState);
...
//                System.out.println("DEBUG: Delta-V magnitude = " + eqDvMagnitude + " m/s");
                ...
                SpacecraftState eqPreState = new SpacecraftState(initialOrbit, initialMass);
                logState("pre", eqPreState);

                eqManProp.addEventDetector(eqBurn);

                AbsoluteDate eqPostDate = eqBurnDate.shiftedBy(0.001);
                SpacecraftState statePostBurn = eqManProp.propagate(eqPostDate);
                double eqDeltaInc = INC_2 - currentIncRad;
                double eqDvMagnitude = 2.0 * speedEq * FastMath.sin(Math.abs(eqDeltaInc)/2.0);
                eqDvUsed = eqDvMagnitude;
                System.out.println("DEBUG: Delta-V magnitude = " + eqDvUsed + " m/s");
                System.out.println("DEBUG: Mass before burn = " + initialMass);
                eqMassAfter = calculateFinalMass(initialMass, eqDvUsed, ISP, g0);
                System.out.println("DEBUG: Mass after burn = " + eqMassAfter);
                statePostBurn = new SpacecraftState(statePostBurn.getOrbit(),eqMassAfter);

                logState("post", statePostBurn);
                SpacecraftState eqFinalState = eqManProp.propagate(eqPostDate);
//                logState("eqfinalstate", eqFinalState);
...
//                double eqMassAfter = calculateFinalMass(initialMass, eqDvUsed, ISP, g0);
                ...
                System.out.printf("Raw final inc = %.12f°\n", FastMath.toDegrees(statePostBurn.getOrbit().getI()));
                Utils.logResults(resultFileName, statePostBurn, statePostBurn.getOrbit(), this.m0, DRYMASS);
                writeManeuverTimestamp(postManeuverDateFileName, statePostBurn.getDate());
                writeManeuverTimestamp(lastManeuverDateFile, statePostBurn.getDate());
                mqttService.sendFileViaMQTT(resultFileName, publishTopic);
...
//        validateManeuverParameters(initialDate, apsideDate, initialMass);
        ...
        if (currentInc <= incEps && ECC >= eccThreshold) {
            ...
            // 1) find *exact* next apogee
            AbsoluteDate apogeeDate = findNextApogee(initialState);

// 2) state at apogee
            SpacecraftState apoState = propagateToDate(initialOrbit, apogeeDate);
            Vector3D rHat   = apoState.getPVCoordinates().getPosition().normalize();
            Vector3D vApo   = apoState.getPVCoordinates().getVelocity();
            double   deltaI = INC_2 - new KeplerianOrbit(apoState.getOrbit()).getI();

// 3) rotate vApo around r̂ by +deltaI  (Rodrigues)
            Vector3D vRot = vApo.scalarMultiply(FastMath.cos(deltaI))
                    .add(rHat.crossProduct(vApo).scalarMultiply(FastMath.sin(deltaI)));
// 4) pure plane‑change ΔV
            Vector3D deltaVVector = vRot.subtract(vApo);

// 5) fire *exactly* at apogee
            DateDetector burnTrigger = new DateDetector(apogeeDate);
            ImpulseManeuver singleBurn = new ImpulseManeuver(burnTrigger, deltaVVector, ISP);

// 6) propagate a millisecond after the burn if you need a “post‑burn” state
            KeplerianPropagator mp = new KeplerianPropagator(initialOrbit);
            mp.addEventDetector(singleBurn);
            SpacecraftState stateAfterBurn = mp.propagate(apogeeDate.shiftedBy(0.001));
...
//                double eqMassAfter = calculateFinalMass(initialMass, eqDvUsed, ISP, g0);
                ...
                System.out.printf("Raw final inc = %.12f°\n", FastMath.toDegrees(statePostBurn.getOrbit().getI()));
                Utils.logApsideDate(apsideFile, statePostBurn.getDate());
