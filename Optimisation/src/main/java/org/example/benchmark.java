...
public class benchmark {
    ...
    public static void main(String[] args) {
        ...
        double iT = FastMath.toRadians(0.05);
        double wT = FastMath.toRadians(0);
        ;
        double WT  = FastMath.toRadians(0);
        double LVT = FastMath.toRadians(0);
        ...
        double       time    = 0;
        AbsoluteDate newDate = initdate.shiftedBy(time);

        Orbit orbit_init =
                new EquinoctialOrbit(a0, ex0, ey0, hx0, hy0, LV0, PositionAngle.TRUE, GCRF, newDate,
        ...
        TimeStampedDouble            shiftOrbit   = new TimeStampedDouble(86400, newDate);
        ArrayList<TimeStampedDouble> noThrustSlot = new ArrayList<>();
        noThrustSlot.add(shiftOrbit);

        QLawWithConstraintPropagatorwithPhasingwithConstraint qlaw =
                new QLawWithConstraintPropagatorwithPhasingwithConstraint(orbit_init, orbit_target,
                                                                          QLawWithConstraintPropagatorwithPhasingwithConstraint::keplerianPropagator,
                                                                          thrust, ISP, mass, 0., 1, 10000,
                                                                          0.1);
        QLawWithConstraintPropagatorwithPhasingwithConstraint.Output Qsolve = qlaw.solve();
        ...
        System.out.println(orbit_init.getDate());
        for (int i = 0; i < 180; i++) {
            System.out.println(Qsolve.maneuvers.get(i).getStartDate() + "Manoeuvre" + i);
        }
        ...
        System.out.println(Qsolve.getTransfertTime() + "  time transfert");
        System.out.println(
        ...
        List<Orbit> orbitListTarget = new ArrayList<Orbit>();
    ...
    private static void addL2EvolutiontoOutput(final StringBuilder dataL2Error,
                                               final QLawWithConstraintPropagatorwithPhasingwithConstraint.Output Qsolve) {
